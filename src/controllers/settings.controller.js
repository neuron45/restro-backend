...
exports.addTax = async (req, res) => {
  try {
    ...
    if (!(title && taxRate && type && taxGroupId)) {
    ...
    const taxId = await addTaxDB(title, taxRate, type, taxGroupId, tenantId);
    return res.status(200).json({
      success: true,
...
exports.getAllTaxGroups = async (req, res) => {
  try {
    const tenantId = req.user.tenant_id;
    const result = await getTaxGroupsDB(tenantId);
    return res.status(200).json(result);
  } catch (error) {
    console.error(error);
    return res.status(500).json({
      success: false,
      message: 'Something went wrong getting tax groups! Please try later!',
    });
  }
};
...
exports.addTaxGroup = async (req, res) => {
  try {
    const tenantId = req.user.tenant_id;
    const title = req.body.title;
    ...
    const taxId = await addTaxGroupDB(title, tenantId);
    return res.status(200).json({
      success: true,
      message: `Tax Group Added.`,
      taxId,
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({
      success: false,
      message: 'Something went wrong adding tax group! Please try later!',
    });
  }
};
...
exports.updateTaxGroup = async (req, res) => {
  try {
    ...
    await updateTaxGroupDB(taxGroupId, title, tenantId);
    return res.status(200).json({
      success: true,
      message: `Tax Details Updated.`,
      taxId,
...
exports.deletTaxGroup = async (req, res) => {
  try {
    const tenantId = req.user.tenant_id;
    const taxGroupId = req.params.id;

    await deleteTaxGroupDB(taxGroupId, tenantId);
    return res.status(200).json({
      success: true,
      message: `Tax Group Detail Removed.`,
      taxId,
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({
      success: false,
      message: 'Something went wrong deleting tax group! Please try later!',
    });
  }
};
