...
exports.createOrderDB = async (
  ...
  try {
    ...
    const [tokenSequence] = await conn.query(
      'SELECT sequence_no, last_updated FROM token_sequences WHERE tenant_id = ? LIMIT 1 FOR UPDATE',
      [tenantId],
    );
    ...
    const [orderResult] = await conn.query(
      `INSERT INTO orders (delivery_type, customer_type, customer_id, table_id, token_no, payment_status, invoice_id, tenant_id, payment_type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        deliveryType,
        customerType,
        customerId,
    ...
    await conn.query(
      'INSERT INTO token_sequences ( sequence_no, last_updated, tenant_id) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE sequence_no = VALUES(sequence_no), last_updated = VALUES(last_updated) ;',
      [tokenNo, today, tenantId],
    );
